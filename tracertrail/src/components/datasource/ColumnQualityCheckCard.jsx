...
export default function ColumnQualityCheckCard({ check }) {
  ...
  const categoryColors = {
    ...
    statistics: "bg-amber-950 text-amber-400 border-amber-800",
    outliers: "bg-rose-950 text-rose-400 border-rose-800",
    patterns: "bg-emerald-950 text-emerald-400 border-emerald-800",
    distribution: "bg-violet-950 text-violet-400 border-violet-800"
  };
          ...
          <div className="flex-1">
            ...
            {check.column_name && (
              <div className="ml-9 mt-1">
                <Badge variant="outline" className="bg-cyan-950 text-cyan-300 border-cyan-800">
        ...
        <CardContent>
          <Tabs defaultValue="sql" className="w-full">
            <TabsList className="grid w-full grid-cols-3 bg-slate-900">
              ...
              <TabsTrigger value="pyspark" className="data-[state=active]:bg-slate-700">
            ...
            <TabsContent value="pyspark" className="mt-3">
              <div className="relative">
                <pre className="bg-slate-900 p-3 rounded-lg text-xs text-slate-300 overflow-x-auto border border-slate-700">
                  <code>{check.pyspark_check || "No PySpark check provided"}</code>
                </pre>
                {check.pyspark_check && (
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => copyToClipboard(check.pyspark_check, "PySpark")}
                    className="absolute top-2 right-2 h-7 w-7 p-0 hover:bg-slate-800"
                  >
                    <Copy className="w-3 h-3 text-slate-400" />
                  </Button>
